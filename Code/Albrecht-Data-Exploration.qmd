...
ggplot(frugivoria_birds, aes(body_size_mm)) + 
  ...
  theme_bw()
```
appears there are a few outliers. 
make a box plot to make sure. 
```{r}
ggplot(frugivoria_birds, aes(body_size_mm)) + 
  geom_boxplot() +
  theme_bw()
...
IUCN status, diet breadth, and human footprint using an oridinal regression. I will do three preliminary analyses comparing acrooss these variables
...
Because I am working with IUCN values again, I am also going to limit this down to the IUCN categories NT, VU, EN, and CR.(basically I am omitting species ranked LC)
This won't change any of the means or the patterns because I am just removing a treatment level. all of the observations for the plots are exactly the same. 
