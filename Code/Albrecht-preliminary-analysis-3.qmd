...
"Error in polr(IUCN_category \~ longevity + body_size_mm + home_range_size + :
attempt to find suitable starting values failed" - it seems to be because the insect diet variable has entries that are 0, the model will not work.
...
Longevity, home range, and percent fruit in diet appeat to not not have a significant impact on IUCN categorization.
